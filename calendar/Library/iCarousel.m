...
- (void)setUp
{
    ...
    _perspective = -1.0/25; // center button size
...
NSComparisonResult compareCustom(NSNumber * one, NSNumber * two, iCarousel *self)
{
    int first = [one intValue];
    int second = [two intValue];
    
  if ((second - first) > 5 || (second - first) < -5) {
      if(second > first)
          return NO;
      else
          return YES;
...
{
    return [[_itemViews allKeys] sortedArrayUsingFunction:(NSInteger (*)(id, id, void *))compareCustom context:(__bridge void *)self];
}
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeCoverFlow2:
        {
            ...
            if(offset == -1 || offset == 1)
             ...
             //   spacing = [self valueForOption:iCarouselOptionSpacing withDefault:0.25];
            ...
            {
                transform = CATransform3DTranslate(transform, x, -25.0, z); // center y
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    /*
    NSArray * arrVisible = [self visibleItemViews];
    if([(UIView *)[arrVisible objectAtIndex:2] isEqual:view])
        offset = 2.0;
    */
    //calculate transform
